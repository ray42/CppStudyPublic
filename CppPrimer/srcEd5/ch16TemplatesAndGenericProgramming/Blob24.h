...
public:
  ...
  // constructor which takes two iterators
...
Blob<T>::Blob(It b, It e):
  data_spt(std::make_shared<std::vector<T>>(b,e))
{rr::greet(__PRETTY_FUNCTION__, this);}
...
/*==========================================================================
 *==========================================================================
 * ConstBlobPtr - custom iterator for const StrBlob
 * Const BlobPtr throws an exception on attempts to access a nonexistent 
 *               element
 *==========================================================================
 *==========================================================================
 */

// as usual, forward declarations for friends
template<typename T>
...
public:
  //////////////////////////////////////////////////////////////////////////
  // constructors///////////////////////////////////////////////////////////
  ConstBlobPtr():wptr(std::weak_ptr<std::vector<T>>()),curr(0)
  { /* empty */}
  ConstBlobPtr(const Blob<T> &sb, size_t pos = 0)
  ...
  //first element
  //CONST ONLY!!!!!!!!!!
  const T& operator[](size_t) const; // TODO

  //////////////////////////////////////////////////////////////////////////
  // some more functions
  
  // dereference
  const T& deref() const; // TODO
  // prefix increment
  ConstBlobPtr& incr(); // TODO
  ...
  // dereference - we will return a reference to the T currently pointed to 
  // by curr.
  // CONST DIFFERENCE!! OTHER ONE IS NONCONST
  const T& operator*() const; // TODO

  // We will return a pointer to the underlying T pointed to by curr.
  // That way, we can access all the T member functions.
  // We cannot return a T object because the T class does not
  // implement operator->(), we can only return a class T& if it implements
  // operator->(), or we must return T*, so it can chain op->.
  // NOTE THE CONST
  const T* operator->() const; // TODO

  // increment, decrement and arithmetic
  ConstBlobPtr& operator++(); // TODO
  ConstBlobPtr& operator--(); // TODO
  ConstBlobPtr operator++(int); // TODO
  ConstBlobPtr operator--(int); // TODO
  ConstBlobPtr& operator+=(size_t); // TODO
  ConstBlobPtr& operator-=(size_t); // TODO
  ConstBlobPtr operator+(size_t); // TODO
  ConstBlobPtr operator-(size_t); // TODO
...
private:
  ...
  std::shared_ptr<std::vector<T>> 
    check(size_t, const std::string&) const; // TODO
...
////////////////////////////////////////////////////////////////////////////
////////////////////////////////////////////////////////////////////////////
// ConstBlobPtr's member functions

// check returns a shared_ptr to the vecor if the check succeeds.
template<typename T>
inline std::shared_ptr<std::vector<T>> 
ConstBlobPtr<T>::check(size_t i, const std::string& msg) const
{
...
//CONST ONLY!!!!!!!!!!
template<typename T>
const T& ConstBlobPtr<T>::operator[](size_t i) const
{
  auto p = check(i, "operator[] out of range");
  return (*p)[i];
}
...
{
  check(curr,"--op past end of ConstBlobPtr");
  ++curr;
  return *this;
}
...
{
  --curr;
  check(curr, "--op before begin of ConstBlobPtr");
  return *this;
}
...
////////////////////////////////////////////////////////////////////////////
////////////////////////////////////////////////////////////////////////////
// ConstBlobPtr's friend functions
template<typename T>
...
template<typename T>
ConstBlobPtr<T> Blob<T>::cbegin()
{
  return ConstBlobPtr<T>(*this,0);
}
template<typename T>
ConstBlobPtr<T> Blob<T>::cend()
{
