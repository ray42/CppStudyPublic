...
class Core
{
  ...
  template<typename T> friend T* clone(const T* tp);

public:
  Core():midterm(0),final(0)
  {
    std::cerr << "Core default constructor" << std::endl;
  }

  Core(std::istream& is)
  {
    read(is);
    std::cerr << "Core stream constructor" << std::endl;
  }
